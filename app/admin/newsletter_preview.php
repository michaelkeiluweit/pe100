<?php
// // oxid eSales © NFQ GmbH 2003
//
// Diese Software ist urheberrechtlich geschuetzt - sie ist
// KEINE Freeware.
//
// Die unerlaubte Verwendung dieser Software ohne gueltigen 
// Lizenzschluessel ist ein ein Verstoss gegen die Lizenz-
// bedingungen und wird straf- bzw. auch zivilrechtlich verfolgt.
//
// http://www.nfq-esales.de

require_once( "../oxconfig.php");
$myConfig->blAdmin = true;
require( "inc/auth.inc.php");
require_once( "oxadminview.php");

class Newsletter_preview extends oxAdminView
{
    function render()
    {   global $myConfig;
    	global $mySession;

        parent::render();
        
        $soxId = $myConfig->getParameter( "oxid");
        if( $soxId != "-1" && isset( $soxId))
        {   // load object
            $oNewsletter = &oxNew( "oxnewsletter", "core");
            $oNewsletter->Load( $soxId);
            $this->aViewData["edit"] =  $oNewsletter;

			// user
            $sUserID = $mySession->getVar( "auth");

            // assign values to the newsletter and show it
            $oNewsletter->Prepare( $this);
            $oNewsletter->Create( $sUserID);
            
            $this->aViewData["previewhtml"] =  $oNewsletter->sHTMLText;
            $this->aViewData["previewtext"] =  $oNewsletter->sPlainText;
        }

        return "newsletter_preview.tpl";
    }

}

?>
