...
<form name="search" id="search" action="[{ $shop->selflink }]" method="post">
    [{ $shop->hiddensid }]
    <input type="hidden" name="cl" value="usergroup_list">
...
<tr>
    <td class="listheader" height="15">&nbsp;&nbsp;&nbsp;<a href="Javascript:document.search.sort.value='oxgroups.oxtitle';document.search.submit();" class="listheader">Beschreibung</a></td>
...
<tr>
    ...
    [{ /if}]
    <td valign="top" class="[{ $listclass}]" height="15"><a href="Javascript:EditThis('[{ $listitem->oxgroups__oxid->value}]');" class="[{ $listclass}]">[{ $listitem->oxgroups__oxtitle->value }]</a></td>
    <td class="[{ $listclass}]">
    [{ if !$listitem->isinternal }]
    <a href="Javascript:DeleteThis('[{ $listitem->oxgroups__oxid->value }]');" class="[{ $listclass}]"><img src="[{$shop->imagedir}]/delete_button.gif" hspace="0" vspace="0" border="0" align="middle" alt="" [{include file="help.tpl" helpid=item_delete}]></a>
    [{/if}]
    </td>
</tr>
[{if $blWhite == "2"}]
