...
{
    blCheck = confirm("ACHTUNG !!!!!!\nWollen Sie diesen Shop wirklich löschen ?");
    if( blCheck == true)
    {   document.search.oxid.value=sID;
...
<tr class="listitem">
    ...
    <input class="listedit" type="text" size="60" maxlength="128" name="where[oxshops.oxname]" value="[{ $where->oxshops__oxname }]">
    </td>
...
<tr>
    <td class="listheader" height="15">&nbsp;&nbsp;&nbsp;<a href="Javascript:document.search.sort.value='oxshops.oxdesc';document.search.submit();" class="listheader">Beschreibung</a></td>
...
<tr>
    ...
    [{ /if}]
    <td valign="top" class="[{ $listclass}]" height="15"><a href="Javascript:EditThis('[{ $listitem->oxshops__oxid->value}]');" class="[{ $listclass}]">[{ $listitem->oxshops__oxname->value }]</a></td>
    <td class="[{ $listclass}]">
    [{ if !$listitem->isinternal }]
    <a href="Javascript:DeleteThis('[{ $listitem->oxshops__oxid->value }]');" class="[{ $listclass}]"><img src="[{$shop->imagedir}]/delete_button.gif" hspace="0" vspace="0" border="0" align="middle" alt="" [{include file="help.tpl" helpid=item_delete}]></a>
    [{/if}]
    </td>
</tr>
[{if $blWhite == "2"}]
